/* ── QUIZ N4 ── uses kotobaN4[] and bunpouN4[] from data.js */

const PREMIUM_TOTAL = 20;
const FREE_TOTAL = 5;
...
function generateQuestions() {
  ...
  const shuffledVocab = _shuffle(kotobaN4);
  const shuffledGrammar = _shuffle(bunpouN4);

  const questions = [];

  shuffledVocab.slice(0, vocabQ).forEach((item) => {
    questions.push(_buildVocabQuestion(item, kotobaN4));
  });

  shuffledGrammar.slice(0, grammarQ).forEach((item) => {
    questions.push(_buildGrammarQuestion(item, bunpouN4));
  });
